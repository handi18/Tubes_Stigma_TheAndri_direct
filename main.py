...
# Setelah keluar dari loop (game over), tampilkan skor akhir:
playerScore = bot.score if hasattr(bot, "score") else 0
setFinalScore({ "name": bot.name if hasattr(bot, "name") else "Player", "score": playerScore })

def setFinalScore(result):
    print(f"Final Score {result['name']}: {result['score']}")
